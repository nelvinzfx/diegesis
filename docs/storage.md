...
Plain JSON/JSONL in app-private storage
(`Context.filesDir`). No ORM, no ksp, no runtime permissions,
no SAF.
...
filesDir/
  ...
  campaigns/
    <campaignId>/
      ...
      turns/<index>.json        one file per turn, zero-padded
...
`turns/<index>.json` — see architecture.md Turn. Variants live in
`variants[]`; each variant keeps synopsis, sceneOutput, checks,
interrupted flag, timestamp.
